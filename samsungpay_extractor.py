...
if  days> 0:
    ...
    status, messages = mailbox.search(None, '(FROM {})'.format("no-reply@samsungpay.com") + ' SINCE ' + since)
else:
    status, messages = mailbox.search(None, '(FROM {})'.format("no-reply@samsungpay.com"))
...
if status == "OK":
    ...
    with open('samsungpay_' + datetime.now().strftime('%m-%d-%Y_%H%M%S') + '.csv', 'w', newline='') as csv_file:
