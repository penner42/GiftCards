...
class ExtractFrame(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        Button(self, text='Show',command=self.extract).grid(row=len(choices), sticky=N+W, pady=4)
        self.update()
    ...
    def extract(self):
        threading.Thread(target=self.extract_real).start()

    def extract_real(self):
        time.sleep(3)
        for c in [c for k, c in self.children.items() if isinstance(c, Checkbutton)]:
            c.configure(state=DISABLED)
        self._queue.put_nowait('1')
        time.sleep(3)
        self._queue.put_nowait('2')
        time.sleep(3)
        for c in [c for k, c in self.children.items() if isinstance(c, Checkbutton)]:
            c.configure(state=ACTIVE)
        self._queue.put_nowait('3')
